...
import ITManagementService from "../../services/fetchRequests/ITManagementService";
      ...
      <View style={styles.employeeInfo}>
        <View style={styles.employeeAvatar}>
          <Text style={styles.employeeAvatarText}>
            {((employee.firstName && employee.firstName[0]) || (employee.username && employee.username[0]) || "I").toUpperCase()}
          </Text>
        </View>
...
const ITEmployeeManagement = ({ state }) => {
  ...
  const fetchITStaff = async (isRefresh = false) => {
    if (isRefresh) {
      setRefreshing(true);
    } else {
      setLoading(true);
    ...
    try {
      const result = await ITManagementService.getITStaff(state.currentUser.token);
      if (result.success) {
        setItStaff(result.itStaff || []);
      } else {
        setError(result.error || "Failed to load IT staff");
      }
    } catch (err) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const result = await ITManagementService.createITEmployee(
        state.currentUser.token,
        {
      ...
      if (result.success) {
        setSuccess("IT employee created successfully! A welcome email has been sent.");
      ...
      setFormErrors({ submit: "Failed to create IT employee" });
    } finally {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const result = await ITManagementService.updateITEmployee(
        state.currentUser.token,
        selectedEmployee.id,
        {
          firstName: formData.firstName.trim(),
      ...
      if (result.success) {
        setSuccess("IT employee updated successfully!");
      ...
      setFormErrors({ submit: "Failed to update IT employee" });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await ITManagementService.removeITEmployee(
        state.currentUser.token,
        selectedEmployee.id
      );

      if (result.success) {
        setSuccess("IT employee removed successfully!");
      ...
      setError("Failed to remove IT employee");
      setShowDeleteModal(false);
    } finally {
      setIsSubmitting(false);
    }
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading IT staff...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>IT Management</Text>
      </View>
      ...
      <Pressable style={styles.addButton} onPress={handleOpenAddModal}>
        ...
        <Text style={styles.addButtonText}>Add IT Employee</Text>
      </Pressable>
      ...
      <View style={styles.section}>
        ...
        {itStaff.length === 0 ? (
          <View style={styles.emptyState}>
            <Icon name="desktop" size={48} color={colors.text.tertiary} />
            <Text style={styles.emptyStateTitle}>No IT Employees</Text>
            <Text style={styles.emptyStateText}>
              Add your first IT employee to help manage technical support and disputes.
            </Text>
          </View>
        ) : (
          <View style={styles.staffList}>
            {itStaff.map((employee) => (
              <EmployeeCard
                key={employee.id}
                employee={employee}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Add IT Employee</Text>
              <Pressable
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Edit IT Employee</Text>
              <Pressable
                style={styles.closeButton}
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.deleteModalContent}>
            ...
            <Text style={styles.deleteTitle}>Remove IT Employee?</Text>
...
export default ITEmployeeManagement;
