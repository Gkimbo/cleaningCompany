...
const AppointmentDetailsPage = ({ state }) => {
  ...
  useEffect(()=>{
    const response =  FetchData.get(
        ...
        const assignedEmployees = JSON.parse(response.appointment.employeesAssigned)
  ...
  return (
    <View style={UserFormStyles.container}>
      ...
      <View style={homePageStyles.homeTileContainer}>
        ...
        <Text>Employees Needed: {appointment.empoyeesNeeded - assignedEmployees.length}</Text>
              ...
              <Text>Has Been Assigned: {appointment.hasBeenAssigned ? "Yes" : "No"}</Text>
