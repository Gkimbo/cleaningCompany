...
describe("verifyHROrOwner Middleware", () => {
  ...
  describe("Authorization Header Validation", () => {
    it("should return 401 when no authorization header is provided", async () => {
      await verifyHROrOwner(req, res, next);
  ...
  describe("User Validation", () => {
    ...
    it("should return 403 for regular homeowner user", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ error: "HR or Owner access required" });
      expect(next).not.toHaveBeenCalled();
    });
  ...
  describe("Successful Authorization", () => {
    ...
    it("should call next() and set req.user for humanResources type user", async () => {
      const token = jwt.sign({ userId: 2 }, secretKey);
      req.headers.authorization = `Bearer ${token}`;
      const mockHR = {
        id: 2,
      ...
      expect(req.user).toEqual(mockHR);
      expect(next).toHaveBeenCalled();
      expect(res.status).not.toHaveBeenCalled();
    });
  });
