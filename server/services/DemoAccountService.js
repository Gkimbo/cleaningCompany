...
// Import the demo seeder
const { createDemoAccounts } = require("../seeders/demo-accounts");
...
class DemoAccountService {
	 ...
	 * This deletes ALL demo-related data and runs the seeder fresh
	 * @returns {Promise<Object>} Summary of reset operation
	 */
	static async resetDemoData() {
		try {
			console.log("[DemoAccountService] Starting complete demo data reset...");
			...
			if (demoUserIds.length > 0) {
				console.log("[DemoAccountService] Deleting all demo data...");

				// Get all demo appointments first (needed for related deletions)
				...
				// Get all demo homes
				const demoHomes = await Home.findAll({
					where: { userId: { [Op.in]: demoUserIds } },
					attributes: ["id"],
				});
				const demoHomeIds = demoHomes.map(h => h.id);

				// Delete in correct order to respect foreign key constraints

				// 1. Delete CleanerRoomAssignments
				if (demoApptIds.length > 0) {
					const deleted = await CleanerRoomAssignment.destroy({
						where: { appointmentId: { [Op.in]: demoApptIds } },
					});
					deletedCount += deleted;
				}

				// 2. Delete CleanerJobOffers
				const deletedOffers = await CleanerJobOffer.destroy({
					where: {
						[Op.or]: [
							{ cleanerId: { [Op.in]: demoUserIds } },
							...(demoApptIds.length > 0 ? [{ appointmentId: { [Op.in]: demoApptIds } }] : []),
						],
					},
				});
				deletedCount += deletedOffers;

				// 3. Delete MultiCleanerJobs
				if (demoApptIds.length > 0) {
				...
				// 6. Delete HomeSizeAdjustmentRequests
				...
				// 7. Delete EmployeeJobAssignments
				const deletedAssignments = await EmployeeJobAssignment.destroy({
					where: {
						[Op.or]: [
				...
				deletedCount += deletedAssignments;

				// 8. Delete UserReviews
				...
				// 9. Delete UserCleanerAppointments
				if (demoApptIds.length > 0) {
				...
				// 11. Delete UserAppointments (including isDemoAppointment ones)
				...
				// 12. Delete RecurringSchedules
				if (demoHomeIds.length > 0) {
					const deletedSchedules = await RecurringSchedule.destroy({
						where: { homeId: { [Op.in]: demoHomeIds } },
					});
					deletedCount += deletedSchedules;
				}

				// 13. Delete HomePreferredCleaner
				const deletedPreferred = await HomePreferredCleaner.destroy({
					where: {
						[Op.or]: [
							{ cleanerId: { [Op.in]: demoUserIds } },
							...(demoHomeIds.length > 0 ? [{ homeId: { [Op.in]: demoHomeIds } }] : []),
						],
					},
				});
				deletedCount += deletedPreferred;

				// 14. Delete CleanerPreferredPerks
				const deletedPerks = await CleanerPreferredPerks.destroy({
					where: { cleanerId: { [Op.in]: demoUserIds } },
				});
				deletedCount += deletedPerks;

				// 15. Delete UserBills
				const deletedBills = await UserBills.destroy({
					where: { userId: { [Op.in]: demoUserIds } },
				});
				deletedCount += deletedBills;

				// 16. Delete UserHomes
				const deletedHomes = await Home.destroy({
					where: { userId: { [Op.in]: demoUserIds } },
				});
				deletedCount += deletedHomes;

				// 17. Delete UserPendingRequests
				const deletedRequests = await UserPendingRequests.destroy({
					where: {
						[Op.or]: [
							{ employeeId: { [Op.in]: demoUserIds } },
							...(demoApptIds.length > 0 ? [{ appointmentId: { [Op.in]: demoApptIds } }] : []),
				...
				// 18. Delete CleanerClient relationships
				const deletedClients = await CleanerClient.destroy({
					where: {
						[Op.or]: [
							{ cleanerId: { [Op.in]: demoUserIds } },
							{ clientId: { [Op.in]: demoUserIds } },
						],
					},
				});
				deletedCount += deletedClients;

				// 19. Delete BusinessEmployee relationships
				const deletedEmployees = await BusinessEmployee.destroy({
					where: {
						[Op.or]: [
							{ businessOwnerId: { [Op.in]: demoUserIds } },
							{ userId: { [Op.in]: demoUserIds } },
				...
				deletedCount += deletedEmployees;

				// 20. Delete BusinessVolumeStats
				const deletedVolumeStats = await BusinessVolumeStats.destroy({
					where: { businessOwnerId: { [Op.in]: demoUserIds } },
				});
				deletedCount += deletedVolumeStats;

				// 21. Finally, delete all demo Users
				const deletedUsers = await User.destroy({
					where: { isDemoAccount: true },
				});
				deletedCount += deletedUsers;

				console.log(`[DemoAccountService] Deleted ${deletedCount} demo records`);
			}

			// Now run the seeder to create fresh demo data
			console.log("[DemoAccountService] Running demo seeder...");
			const createdAccounts = await createDemoAccounts();

			console.log("[DemoAccountService] Demo data reset complete!");
